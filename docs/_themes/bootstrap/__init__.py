...
class HTMLTranslator(SphinxHTMLTranslator):
    ...
    def visit_document(self, node):
        ...
        if self.toc_subnav:
            ...
            self.page_toc_maxdepth = int(
                self.builder.config.html_theme_options.get('page_toc_maxdepth',
                                                           '-1'))
    ...
    def visit_admonition(self, node, name=''):
        self.body.append(self.starttag(
            node, 'div',
            CLASS=('alert %s' % self.admonitionclasses.get(name, ''))))
    ...
    def visit_bullet_list(self, node):
        if self.is_partial and len(node) and not self.bullet_list_is_global(
                node):
            # honor page_toc_maxdepth option
            if -1 < self.page_toc_maxdepth < self.bullet_list_depth(node):
                raise nodes.SkipNode

            # for page toc in the nav, collapse the first list,
            # which contains only
            # a link to the top of the page, into the next list containing the
            # first level of page sections.
            if (
                self.toc_nav or self.toc_sidebar) and \
                    not self.page_toc_handled_first:
                ...
                if len(node.children[0].children) > 1:
                    ...
                    # required for target based scrollspy,
                    # but screws the styling.
                    #if self.toc_nav:
        ...
        if self.compact_simple and not old_compact_simple:
            ...
            if not self.is_partial and self.toc_subnav and node is self\
                .page_toc:
                self.body.append(
                    self.starttag({'classes': ['page-toc', 'subnav', 'navbar']},
                                  'div'))
                self.body.append(
            ...
            # handle toc bullet list
                ...
                # only global toc is in the nav, so see if that is what we
                # are rendering
    ...
    def visit_list_item(self, node):
        ...
        if self.is_partial:
            ...
            if self.toc_nav:
                children = node.children
                if len(children) > 1:
                    if isinstance(children[1], nodes.bullet_list) and len(
                            children[1]):
                        if self.bullet_list_is_global(node) or \
                                        self.page_toc_maxdepth < 0 or \
                                        self.bullet_list_depth(children[
                                            1]) <= self.page_toc_maxdepth:
            ...
            for classname in classes[:]:
                children = node.children
                # only global toc is in the nav, so see if that is what
                # we are rendering
                if not self.toc_nav and classname.startswith(
                        'toctree-l') and len(children) > 1:
                    if isinstance(children[1], nodes.bullet_list) and len(
                            children[1]):
                        classes.append('dropdown-submenu')
